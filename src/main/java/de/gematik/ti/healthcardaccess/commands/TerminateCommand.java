...
/**
 * Commands representing Terminate Command gemSpec_COS#14.2.9
 *
 */
public class TerminateCommand extends AbstractHealthCardCommand {
    private static final int CLA = 0x00;
    private static final int INS = 0xE8;
    private static final int MODE_CURRENT_EF = 0x00;
     ...
     * @param dfSpecific
     */
    public TerminateCommand(final Key key, final boolean dfSpecific) {
        super(CLA, INS);
     ...
     * @param reference
     */
    public TerminateCommand(final byte[] reference) throws IOException {
        super(CLA, INS);
        this.p1 = MODE_PUBLIC_KEY_REFERENCE;
        this.p2 = 0x00;
        this.data = new DERTaggedObject(false, 3, new DEROctetString(reference)).getEncoded();
    }
     ...
     * @param dfSpecific
     */
    public TerminateCommand(final Password password, final boolean dfSpecific) {
        super(CLA, INS);
